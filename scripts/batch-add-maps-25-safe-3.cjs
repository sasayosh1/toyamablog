...
// 安全な中規模バッチ処理（第3回：146-170番目）
async function getNextBatch3Articles(startIndex = 145, batchSize = 25) {
...
function generateSafeMapInfo3(title, slug) {
  const locationMap = {
  ...
  } else if (title.includes('橋')) {
    facilityType = '橋';
    description = `${location.area}の橋で美しい景観を楽しめます`;
  } else if (title.includes('館') || title.includes('博物館')) {
    facilityType = '文化施設';
    description = `${location.area}の文化施設で学びと発見があります`;
  } else if (title.includes('祭') || title.includes('イベント')) {
    facilityType = 'イベント会場';
    description = `${location.area}で開催される特別なイベントです`;
  } else if (title.includes('山') || title.includes('キャンプ')) {
    facilityType = '自然スポット';
    description = `${location.area}の自然スポットでアウトドアを楽しめます`;
  }
...
async function safeBatch25Maps3() {
  try {
    console.log('🛡️ 安全バッチ処理 第3回 - 記事146～170番目を処理');
    console.log('🎯 方針: 確実性最優先、安定した成功パターンを継続');
    
    const articles = await getNextBatch3Articles(145, 25);
    ...
    for (let i = 0; i < articles.length; i++) {
      ...
      try {
        ...
        const mapInfo = generateSafeMapInfo3(existingArticle.title, article.slug?.current || '');
        
        const googleMapBlock = {
          _type: 'html',
          _key: `googlemap-safe-batch3-${Date.now()}-${i}`,
    ...
    console.log(`🎉 累計完了: ${145 + successCount}/206記事 (${Math.round((145 + successCount)/206*100)}%)`);
    
    if (errorCount === 0) {
      console.log('🛡️ 完璧！第3回安全バッチ処理も成功しました');
...
safeBatch25Maps3().catch(console.error);