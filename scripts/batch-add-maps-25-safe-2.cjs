...
// 安全な中規模バッチ処理（第2回：121-145番目）
async function getNextBatch2Articles(startIndex = 120, batchSize = 25) {
...
function generateSafeMapInfo2(title, slug) {
  const locationMap = {
...
async function safeBatch25Maps2() {
  try {
    console.log('🛡️ 安全バッチ処理 第2回 - 記事121～145番目を処理');
    console.log('🎯 方針: 確実性最優先、前回の成功を継続');
    
    const articles = await getNextBatch2Articles(120, 25);
    ...
    for (let i = 0; i < articles.length; i++) {
      ...
      try {
        ...
        const mapInfo = generateSafeMapInfo2(existingArticle.title, article.slug?.current || '');
        
        const googleMapBlock = {
          _type: 'html',
          _key: `googlemap-safe-batch2-${Date.now()}-${i}`,
    ...
    console.log(`🎉 累計完了: ${120 + successCount}/206記事 (${Math.round((120 + successCount)/206*100)}%)`);
    
    if (errorCount === 0) {
      console.log('🛡️ 完璧！第2回安全バッチ処理も成功しました');
...
safeBatch25Maps2().catch(console.error);