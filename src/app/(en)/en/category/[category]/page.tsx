...
export async function generateMetadata({ params }: { params: Promise<{ category: string }> }): Promise<Metadata> {
  ...
  try {
    ...
    return {
      title: 'Invalid URL - Do you like Toyama?',
      description: 'The specified URL is in an invalid format.',
      robots: { index: false, follow: false }
    }
  }

  const canonical = `https://sasakiyoshimasa.com/en/category/${encodeURIComponent(decodedCategory)}`
  ...
  return {
    title: `${decodedCategory} - Do you like Toyama?`,
    description: `Articles about ${decodedCategory} in Toyama Prefecture. Delivering local information linked with YouTube Shorts.`,
    robots: postCount > 0 ? undefined : { index: false, follow: true },
    openGraph: {
      title: `${decodedCategory} - Do you like Toyama?`,
      description: `Articles about ${decodedCategory} in Toyama Prefecture`,
      type: 'website',
      url: canonical,
    },
    alternates: {
          ...
          {categoryPosts.length > 0 ? (
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 lg:gap-8">
              {categoryPosts.map((post) => (
                <PostCard key={post._id} post={post} locale="en" basePath="/en" />
              ))}
            </div>
          ) : (
              ...
              <div className="flex items-start gap-4">
                ...
                <div>
                  ...
                  <p className="text-gray-600 leading-relaxed">
                    Articles may be added to this category later. In the meantime, please check out other categories.
                  </p>
