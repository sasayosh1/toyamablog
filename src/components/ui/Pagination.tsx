...
export default function Pagination({ currentPage, totalPages, basePath = '', locale = 'ja' }: PaginationProps) {
  ...
  return (
    <nav className="flex justify-center items-center space-x-2 mt-12 mb-8" aria-label={locale === 'en' ? 'Pagination' : 'ページネーション'}>
      {currentPage > 1 && (
        <Link
          ...
          aria-label={locale === 'en' ? 'Previous page' : '前のページ'}
        >
      ...
      {pageNumbers.map((page, index) => {
        ...
        return (
          <Link
            ...
            aria-label={locale === 'en' ? `Page ${pageNum}` : `ページ${pageNum}`}
      ...
      {currentPage < totalPages && (
        <Link
          ...
          className="flex items-center px-3 py-2 text-sm font-medium text-gray-500 bg-white border border-gray-300 rounded-md hover:bg-gray-50 hover:text-gray-700 transition-colors"
          aria-label={locale === 'en' ? 'Next page' : '次のページ'}
        >
          {locale === 'en' ? 'Next' : '次へ'}
