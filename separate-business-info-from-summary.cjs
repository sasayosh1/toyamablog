...
async function separateBusinessInfoFromSummary() {
  try {
    console.log('🔧 まとめセクションから営業情報・アクセスを分離中...');
    console.log('正確な構造: まとめ（単独） → マップ → 営業情報・アクセス（独立） → タグ');
    console.log('');
    ...
    const correctStructure = [
      ...
      // 営業情報・アクセス（完全分離・独立セクション）
      {
        _type: 'block',
        _key: 'business-access-separated',
        style: 'normal',
        children: [{
          _type: 'span',
          _key: 'business-access-separated-span',
          text: '**営業情報・アクセス**\n\n📍 **所在地**: 富山県氷見市中央町（氷見駅から徒歩12分）\n🚗 **駐車場**: 無料駐車場完備（普通車25台、大型車3台対応）\n🕐 **営業時間**: 9:00-18:00（土日祝は19:00まで延長営業）\n📅 **定休日**: 年中無休（年末年始のみ休業）\n💰 **価格帯**: 小物500円-1,500円、アパレル2,500円-4,000円\n📱 **SNS**: Instagram @himitsu_no_asobiba（フォロワー約6,200人）\n💳 **決済方法**: 現金・各種クレジットカード・PayPay・d払い・auPay対応\n🚃 **電車アクセス**: JR氷見線氷見駅より徒歩12分\n🚌 **バスアクセス**: 氷見市コミュニティバス「中央町」停留所より徒歩3分',
          marks: []
        }],
        markDefs: []
      }
    ...
    console.log('✅ まとめセクションから営業情報・アクセスの分離完了');
    console.log('');
    
    // 文字数カウント
    let totalChars = 0;
    correctStructure.forEach(block => {
    ...
    console.log('   ✅ まとめセクションから営業情報・アクセスを完全分離');
    console.log('   ✅ まとめセクション：行動促進内容のみに純化');
    console.log('   ✅ 営業情報・アクセス：独立セクションとしてマップ直下に配置');
    console.log('   ✅ 正確な構造順序：まとめ → マップ → 営業情報・アクセス → タグ');
    console.log('');
...
separateBusinessInfoFromSummary();