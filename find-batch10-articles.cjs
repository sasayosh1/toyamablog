...
async function findBatch10Articles() {
  ...
  console.log('=== 第10バッチ候補（第46-50記事）===');
  const tenthBatch = candidates.slice(45, 50); // 46番目から50番目まで
  
  tenthBatch.forEach((article, index) => {
    console.log(`第${index + 46}記事:`);
...
findBatch10Articles().catch(console.error);