...
async function findBatch9Articles() {
  ...
  console.log('=== 第9バッチ候補（第41-45記事）===');
  const ninthBatch = candidates.slice(40, 45); // 41番目から45番目まで
  
  ninthBatch.forEach((article, index) => {
    console.log(`第${index + 41}記事:`);
...
findBatch9Articles().catch(console.error);