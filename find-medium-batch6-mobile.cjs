...
async function findMediumBatch6Articles() {
  ...
  console.log('=== 中記事第6バッチ候補（第26-30記事）- スマホ最適化版 ===');
  const sixthBatch = candidates.slice(25, 30); // 26番目から30番目まで
  
  sixthBatch.forEach((article, index) => {
    console.log(`第${index + 26}記事:`);
  ...
  console.log(`処理済み: 25記事`);
  console.log(`第6バッチ対象: 5記事`);
  console.log(`残り: ${candidates.length - 30}記事`);
}

findMediumBatch6Articles().catch(console.error);