...
async function findBatch11Articles() {
  ...
  console.log('=== 第11バッチ候補（第51-55記事）===');
  const eleventhBatch = candidates.slice(50, 55); // 51番目から55番目まで
  
  eleventhBatch.forEach((article, index) => {
...
findBatch11Articles().catch(console.error);