...
async function fixHimiArticleStructure() {
  try {
    console.log('🔧 氷見市記事をクラウドルール準拠構造に修正中...');
    console.log('新構造: H1タイトル → 動画 → 見出し → もくじ → H2本文記事 → まとめ → マップ → 営業情報・アクセス → タグ');
    console.log('');
    ...
    const completeContent = [
      ...
      {
        _type: 'block',
        _key: 'business-access-info',
        style: 'normal',
        children: [{
          _type: 'span',
          _key: 'business-access-span',
    ...
    console.log('✅ 氷見市記事のクラウドルール準拠構造修正完了');
    console.log('');
    
    // 文字数カウント
    let totalChars = 0;
    completeContent.forEach(block => {
    ...
    console.log('🔧 追加した要素（クラウドルール準拠）:');
    console.log('   ✅ Googleマップセクション（まとめ後、営業情報前）');
    console.log('   ✅ 営業情報・アクセスセクション（マップ後、タグ前）');
    console.log('   ✅ 箇条書き活用による視覚的構成');
    console.log('   ✅ 絵文字付き情報整理');
    console.log('   ✅ 正確な記事構造順序');
    console.log('');
