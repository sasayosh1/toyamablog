...
### 2025年11月11日: トップページ記事サムネイル表示問題とYouTubeデータ不整合修正
- **問題**: トップページ上位18記事のサムネイルが表示されず、SVGフォールバック画像が表示される
- **発見経緯**: ユーザーからスクリーンショット提示でサムネイル表示問題を指摘
- **重大性**: 記事一覧の視認性とユーザビリティに直接影響する重要問題
- **根本原因**: Sanityデータ構造の不整合
...
- **対応内容** (コミット: c5fcee6):
  ...
  2. **フロントエンド修正** (`src/components/ui/PostCard.tsx:170-171`)
     - YouTubeURL取得ロジックを更新: `const youtubeUrl = post.youtubeUrl || post.youtubeVideo?.url;`
     - `youtubeVideo` インターフェース定義を追加（TypeScript型安全性）
     - 両方のフィールドパターンに対応可能に
  3. **Sanityクエリ修正** (`src/lib/sanity.ts:106`)
     - `getPostsPaginated` 関数のクエリに `youtubeVideo` フィールドを追加
     - フロントエンドが必要なデータを確実に取得
  4. **メンテナンススクリプト強化** (`scripts/maintenance.cjs`)
     - **検出機能**: YouTubeデータ不整合チェックを追加
     - **自動修正**: `youtubeVideo.url` から `youtubeUrl` への自動コピー機能
     - **レポート**: 該当記事の一覧と詳細情報を出力
     - 週次メンテナンスワークフローで自動実行（毎週月曜AM3:00 JST）
- **技術的詳細**:
  - **データ不整合の原因**: 記事作成スクリプトが本来両方のフィールドを設定すべきところ、何らかの理由で `youtubeUrl` が設定されなかった
  - **後方互換性**: 修正により以下の全パターンに対応
    - ✅ `youtubeUrl` のみ存在
    - ✅ `youtubeVideo.url` のみ存在
    - ✅ 両方存在
    - ✅ どちらも存在しない場合はSVGフォールバック
  - **予防的監視**: メンテナンススクリプトで継続的に監視・自動修正
- **効果**:
  - トップページ18記事のサムネイルが正常表示
  - YouTubeサムネイル取得の信頼性向上
  - 今後の同様問題を自動検出・修正可能
- **検証**: http://localhost:4000 で正常動作確認済み
- **教訓**: **Sanityデータ構造の不整合は表示問題の根本原因となる。診断スクリプトによる詳細分析が問題解決の鍵**
- **予防策**: 週次メンテナンスでデータ品質を継続的に監視
