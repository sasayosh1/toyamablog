...
- **変更前必須チェック**: コード変更前に必ず画像表示に影響する部分をチェックし、影響がある場合は代替手段を検討する
- **完全性維持**: パフォーマンス改善、機能追加、バグ修正等のいかなる理由でも画像表示機能を破壊してはならない
- **即座復旧**: もし画像が消えた場合は、他の作業を停止して最優先で画像表示を復旧する
- **予防的保護**: 画像関連のコード変更時は、変更前後で必ず動作確認を行い、問題があれば変更を取り消す
- **ゼロトレランス**: 「指示の副作用で画像が消える」事象の発生をゼロにする