...
### 2025年9月26日: Sanity Studioプレビューボタン削除絶対禁止ルール制定
- **重大性**: Sanity Studioプレビューボタンは記事管理の最重要機能
- **絶対禁止事項**: プレビューボタンの削除・無効化・変更は最重要インシデント
- **保護対象**:
  - `/src/sanity/actions/PreviewAction.tsx` - プレビューボタンコンポーネント
  - `sanity.config.ts`のDocument Actions設定
  - プレビューボタンに関連するすべてのファイル・設定
- **実装詳細**:
  - Document Actions APIを使用してSanity Studio右側アクションメニューに配置
  - 開発環境: `http://localhost:3002/blog/{slug}`
  - 本番環境: `https://sasakiyoshimasa.com/blog/{slug}`
  - スラッグ未設定時の適切なエラーメッセージ表示
- **絶対ルール**:
  - **削除・無効化の完全禁止**: いかなる理由でもプレビューボタンを削除・無効化してはならない
  - **変更時事前承認**: 機能追加・改善時も事前承認なしの変更は禁止
  - **動作確認必須**: 変更後は必ずプレビュー機能の動作確認を実行
- **緊急対応プロトコル**: プレビューボタンが表示されない・動作しない場合は最優先で復旧
- **教訓**: **プレビューボタンはブログ管理の生産性を大幅に向上させる最重要機能**
- **対策**: プレビューボタン関連コードへの変更は最高レベルの注意で実行
...
### 2025年9月17日: 汎用カテゴリ「自然・公園」設定インシデント（継続違反）
- **問題**: 既存記事で禁止カテゴリ「自然・公園」が使用され続けている重大違反
- **発見記事**: 氷見市ヒミツアツビハ記事（localhost:3000/blog/himi-city-1757253039364）
- **違反内容**:
  - URLに「himi-city」と地域名が明記されているにも関わらず
  - カテゴリが「自然・公園」（禁止された汎用カテゴリ）に設定
  - 正しくは【氷見市】をカテゴリとすべき
- **重大性**: CLAUDE.mdルール295-301行目の【最重要】カテゴリ設定規則の完全違反
- **即座の対応**:
  - Sanity Studio経由で該当記事のカテゴリを「氷見市」に修正完了
  - Save & Publishで正常に公開
- **根本原因**: 過去のYouTube記事作成時のカテゴリ自動設定ロジック問題が未修正のまま残存
- **予防策再確認**:
  - **汎用カテゴリ完全禁止**: 「自然・公園」「グルメ」「観光」等は絶対使用禁止
  - **地域名カテゴリ必須**: 【】内の地域名を必ずカテゴリとして使用
  - **既存記事監査**: 他にも汎用カテゴリが残存していないか全記事チェック必要
- **緊急対応完了**:
  - 「自然・公園」→「氷見市」への修正完了
  - サイト表示の正常性確認済み
- **教訓**: **既存記事の継続監視と汎用カテゴリの完全根絶が必要**
- **継続課題**: 全記事の包括的カテゴリ監査実施
...
### 箇条書きの使用規則（2025年9月29日改定）
- **箇条書きは過度に使用しない**：単語や短文が横に連続すると読みづらくなるため、限定的に使用
- **使用条件**：多くの単語や短い項目が連続する場合に限り、読みやすさ向上のために使用
- **原則**：通常の文章での記述を優先し、箇条書きは必要最小限に留める
- **モバイル読者配慮**：箇条書きが多すぎると逆に見づらくなることを考慮
...
**注意**：アクセス情報やおすすめポイントなどは、通常の文章で自然に記述することを優先する

### 記事構成と文字数
- **導入文**: 200〜300字で記事の魅力を簡潔に
- **H2見出し**: 内容に合わせて柔軟に設定（2〜4つ程度）
- **H3見出し**: 必須ではなく、文章上どうしても必要な場合のみ使用
- **H4見出し**: 記事本文では使用禁止（H2・H3のみ使用可能）
- **文字数ルール（2025年10月5日制定）**:
  - **基本文字数**: 最低1000文字以上
...
- **本文記事**: H2見出しで構成されたメインコンテンツ（H3は必須ではなく、文章上どうしても必要な場合のみ使用、H4は使用禁止）
- **まとめセクション（厳格ルール）**: 
...
### アフィリエイトシステム運用規則（厳格遵守）
- **アフィリエイトコード絶対保護（最重要事項）**:
  - **改変完全禁止**: ユーザーから提供されるアフィリエイトコードは一文字たりとも改変しない
  - **原文完全維持**: URL、パラメータ、タグID、その他すべての要素を原文のまま使用
  - **最重要インシデント**: アフィリエイトコードの改変は収益関連設定変更と同等の最重要インシデント
  - **確認義務**: 実装前後でアフィリエイトコードが完全に一致していることを必ず確認
  - **例外なし**: パフォーマンス向上、SEO最適化、コード整理等のいかなる理由でも改変禁止
- **パフォーマンス最優先**: 軽量化されたOptimizedコンポーネントを優先使用
  - `OptimizedAffiliateLink` - 高パフォーマンス単体リンク
  - `OptimizedAffiliateText` - 軽量テキストリンク
  - React.memo、useMemo活用で再レンダリング最小化
- **画像最適化必須**: Next.js Image コンポーネント使用、lazy loading、適切なサイズ設定
- **SEO対応必須**:
  - `rel="nofollow noopener noreferrer"`の設定
  - 構造化データ（schema.org/Product）の実装
  - 適切なalt属性とaria-label設定
- **透明性確保必須**:
  - [PR]ラベルの表示
  - アフィリエイト表示文の配置
  - 免責事項の記載
- **プラットフォーム別最適化**:
  - Amazon: オレンジ系カラー
  - 楽天市場: レッド系カラー
  - Yahoo!ショッピング: パープル系カラー
  - 汎用: ブルー系カラー
- **コンポーネント使い分け**:
  - 商品紹介カード: `OptimizedAffiliateLink`
  - 文章内リンク: `OptimizedAffiliateText`
  - 複数商品比較: `AffiliateLinks`（grid/horizontal レイアウト）
