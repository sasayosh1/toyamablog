...
async function cleanYouTubeDescriptions() {
  try {
    console.log('🧹 YouTube概要欄引用のクリーンアップを開始...');
    ...
    for (const post of posts) {
      ...
      if (hasChanges && cleanedBody.length !== originalLength) {
        ...
        if (cleanedCount % 10 === 0) {
          console.log(`⏱️ ${cleanedCount}件処理完了、1秒待機中...`);
...
cleanYouTubeDescriptions().then(count => {
